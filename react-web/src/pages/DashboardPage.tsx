  ...
  return (
    <div>
      ...
      {loading || !summary ? (
        ...
        <>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 24, maxHeight: 200, overflowY: 'auto', overflowX: 'hidden', paddingRight: 8 }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(8, 1fr)', gap: 12 }}>
              <StatCard label="Orders" value={summary.counts.orders} />
              <StatCard label="Revenue (Completed)" value={`₹${summary.total_revenue.toFixed(2)}`} accent="var(--color-success)" />
              <StatCard label="Products" value={summary.counts.products} />
              <StatCard label="Low Stock" value={summary.counts.low_stock_products} accent={summary.counts.low_stock_products > 0 ? 'var(--color-warning)' : undefined} />
              <StatCard label="Warehouses" value={summary.counts.warehouses} />
              <StatCard label="Suppliers" value={summary.counts.suppliers} />
              <StatCard label="Farmers" value={summary.counts.farmers} />
              <StatCard label="Customers" value={summary.counts.customers} />
            </div>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 16, marginBottom: 24 }}>
            <ChartCard title="Orders by Status" data={summary.order_status_breakdown} dataKey="count" nameKey="status" />
            <ChartCard title="Inventory by Category" data={summary.inventory_by_category} dataKey="quantity" nameKey="category" />
            <ChartCard title="Stock by Warehouse" data={summary.stock_by_warehouse} dataKey="quantity" nameKey="warehouse" />
          </div>
